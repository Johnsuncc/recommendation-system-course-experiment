...
# 推荐n=20个最热门物品的baseline
# precision=11%, recall=4%, coverage=0.2%, popularity=169
# time: 17s
...
if __name__ == "__main__":
    ...
    # 读取数据
    rating_file = '../dataset/ratings.csv'
    ...
    for item, users in item_users.items():
        item_counts[item] = len(users)
    # 获取最热门的n部电影
    rank_base = sorted(item_counts.items(), key=itemgetter(1), reverse=True)[0:n]
    # baseline精度
    hit = 0
    total = 0
    for user in test.keys():
        for item, _ in rank_base:
            if item in test[user]:
                hit += 1
        total += n
    print("baseline precision: ", hit / total)
    # baseline召回率
    hit = 0
    total = 0
    for user in test.keys():
        for item, _ in rank_base:
            if item in test[user]:
                hit += 1
        total += len(test[user])
    print("baseline recall: ", hit / total)
    # baseline覆盖率
    recommend_items = set()
    total_items = set()
    for user in train.keys():
    ...
    for item, _ in rank_base:
        recommend_items.add(item)
    print("baseline coverage: ", len(recommend_items) / len(total_items))
    # baseline流行度
    sum_popularity = 0
    for _, num in rank_base:
        sum_popularity += num
    print("baseline popularity: ", sum_popularity / n)
